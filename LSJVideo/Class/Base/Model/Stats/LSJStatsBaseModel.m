...
@interface LSJStatsInfoMeta : DBPersistence
...
@implementation LSJStatsInfoMeta
@synthesize currentId = _currentId;

+ (instancetype)sharedMeta {
    static LSJStatsInfoMeta *_sharedMeta;
...
@implementation LSJStatsInfo

//- (instancetype)init {
//    self = [super init];
//    if (self) {
//        _statsId = @([LSJStatsInfoMeta sharedMeta].nextId);
//        _appId = LSJ_REST_APP_ID;
//        _pv = LSJ_REST_PV.stringValue;
//        _userId = [LSJUtil userId];
//        _osv = @"iv";
//    }
//    return self;
//}
...
@implementation LSJStatsResponse
...
@implementation LSJStatsBaseModel

+ (Class)responseClass {
    return [LSJStatsResponse class];
}

- (NSDictionary *)encryptWithParams:(NSDictionary *)params {
    //    NSDictionary *signParams = @{  @"appId":LSJ_REST_APP_ID,
    //                                   @"key":kSignKey,
...
- (NSURL *)baseURL {
    return [NSURL URLWithString:LSJ_STATS_BASE_URL];
}
...
- (NSArray<NSDictionary *> *)validateParamsWithStatsInfos:(NSArray<LSJStatsInfo *> *)statsInfos shouldIncludeStatsType:(BOOL)includeStatsType {
    NSArray<LSJStatsInfo *> *validStatsInfos = [statsInfos bk_select:^BOOL(LSJStatsInfo *statsInfo) {
        if (statsInfo.userId.length == 0) {
            statsInfo.userId = [LSJUtil userId];
        }
    ...
    [statsInfos enumerateObjectsUsingBlock:^(LSJStatsInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [statsTypes enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        [validStatsInfos enumerateObjectsUsingBlock:^(LSJStatsInfo * _Nonnull statsInfo, NSUInteger idx, BOOL * _Nonnull stop) {
